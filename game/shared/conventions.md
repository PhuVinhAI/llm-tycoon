...
The CLI's `build` command assembles the Game Documentation into two playable files, following the MANIFEST declared in `src/build/manifest.ts`: `build/LLM-TYCOON.md` (the player build — contains nothing from `dev/` and never mentions Dev Mode) and `build/LLM-TYCOON-DEV.md` (the development build — the player build plus the `dev/` modules).
...
Run `bun run build` after any documentation change. Never edit the build output directly.
