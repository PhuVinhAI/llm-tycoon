...
<!--
Tiếng Việt (tóm tắt):
Dự án LLM là Model Project đặc biệt: pretrain một mô hình ngôn ngữ lớn trên
dữ liệu quy mô web. Mọi luật Model Project vẫn áp dụng trừ phần ghi đè:

Điều kiện khởi động: sở hữu SCALE (kéo theo PRET + kiến trúc PTRF); một
Dataset web-mixed Size 5, Quality ≥ 3; cam kết M ≥ 4 tháng với CU/tháng × M
≥ 40 CU-month (đã tính giảm trừ từ nhân sự — engine kiểm tra trước khi
chạy); trả trước $5,000 hạ tầng.

Quality: dùng dòng PTRF nhưng yêu cầu compute = 40 CU-month; Task = "LLM
(general)" (Match +10, Demand theo bảng thị trường); cộng thêm +10 điểm
quy mô.

Kết cục: Q ≥ 70 → sự kiện Term Sheet (nhận → THẮNG; từ chối → +5 REP, chơi
tiếp tự do). Q 55–69 → chạy được nhưng demo chưa ấn tượng: +10 REP, phát
hành bình thường. Q < 55 → bài học đắt giá: −3 REP, +20 RP. Được thử lại
không giới hạn; phạt lặp áp dụng nếu dùng lại đúng Dataset cũ.
-->
